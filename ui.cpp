...
ui_rect::ui_rect(SDL_Renderer* renderer, std::string id, float x, float y, float w, float h, float r, SDL_Color default_color,int z) : renderer(renderer), id(id), x(x), y(y), w(w), h(h), r(r), z(z), default_color(default_color) {
	...
	for (int c = 0; c < 4; c++) circles[c] = new ui_circle(renderer, id, (x + r + ((static_cast<int>(c / 2)) * (w - (2 * r)))), (y + r + ((c % 2) * (h - (2 * r)))), r, default_color, z);
}
...
func(func),x(x),y(y),w(w),h(h),default_c(default_c),hover_c(hover_c),press_c(press_c),id(id),font(font),renderer(renderer),z(z),align_c(align_center),textbox(renderer,id,font,font_p,x,y,text,text_c,font_size, z),r(r){updateText();}
...
submit_func(submit_func), maxchar(maxchar), id(id),default_text(default_text), z(z), Button([&]() {}, renderer,id, default_text, font, font_path, x, y, w, h, font_size, default_color, default_color, selected_color, text_color, r, align_center, z) {}
